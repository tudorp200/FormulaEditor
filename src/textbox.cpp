...
TextBox createTextBox(string textString, Font& font, int charSize, int x, int y,
    ...
    int borderThickness) {
    TextBox textbox;
    textbox.fullText = textString; // save the string that is supposed to be shown on the screen

    // initialize the background of the textbox
    textbox.background = createRect(x, y, width, height, bg, border, borderThickness);
